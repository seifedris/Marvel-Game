...
public class Game implements PlayerListener {
	...
	private ArrayList<Cover> availableCovers;
	...
	public Game(Player first, Player second) {
		...
		availableCovers = new ArrayList<Cover>();
		placeChampions();
		placeCovers();
	...
	public static void loadChampions(String filePath) throws IOException {
		...
		br.close();
		for (Champion k : availableChampions) {
	...
	public void placeCovers() {
		...
		while (i < 5) {
			...
			if (board[x][y] == null) {
				...
				if (c.getCurrentHP() % 10 == c1 || c.getCurrentHP() % 10 == c2 || c.getCurrentHP() % 10 == c3
						|| c.getCurrentHP() % 10 == c4) {

				} else {
					board[x][y] = c;
					availableCovers.add(c);
					if (i == 0)
						c1 = c.getCurrentHP() % 10;
					if (i == 1)
						c2 = c.getCurrentHP() % 10;
					if (i == 2)
						c3 = c.getCurrentHP() % 10;
					if (i == 3)
						c4 = c.getCurrentHP() % 10;
					i++;
				}
			}
		}
	...
	public ArrayList<Cover> getAvailableCovers() {
		return availableCovers;
	}

	public void setAvailableCovers(ArrayList<Cover> availableCovers) {
	...
	public void onUpdatePlayground() {

		if (listener != null)
			listener.onUpdatePlayground(getBoard());
	}

	public void onUpdateChampions() {
		if (listener != null)
			listener.onUpdateChampions(firstPlayer, secondPlayer);
